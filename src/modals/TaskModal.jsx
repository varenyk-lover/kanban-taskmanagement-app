...
const TaskModal = ({colIndex, taskIndex, setIsTaskModalOpen}) => {
    ...
    return (
        <div onClick={onClose}
             className='fixed right-0 left-0 top-0 py-4 overflow-scroll scrollbar-hide z-50 bottom-0 justify-center
        items-center flex bg-[#00000080]'>
            ...
            <div
                ...
                <div className='mt-8 flex flex-col space-y-3'>
                    ...
                    <select className='select-status flex-grow px-4 py-2 rounded-md
                    text-sm bg-transparent focus:border-0 border border-gray-300 focus:outline-[#635fc7]
                    outline-none' value={status} onChange={onChange}>
            ...
            {isDeleteModalOpen && (
                <DeleteModal
                    ...
                    onDeleteBtnClick={onDeleteBtnClick} title={task.title} type='task'/>
            )}

            {isAddTaskModalOpen && (
                <AddEditTaskModal setOpenAddEditTask={setIsAddTaskModalOpen} type='edit' taskIndex={taskIndex}
                                  prevColIndex={colIndex} setIsTaskModalOpen={setIsTaskModalOpen}/>
            )}
        </div>
    )
};
